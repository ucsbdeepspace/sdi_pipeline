...
def subtract(hduls, name="ALGN", method: ("ois", "numpy") = "ois"):
    ...
    template = combine(hduls, name)[
        "PRIMARY"
    ].data  # this is a temporary HDUL containing 1 PrimaryHDU with combined data
    if method == "ois":
        for i, hdu in enumerate(hduls):
            try:
                diff = ois.optimal_system(
                    image=hdu[name].data, refimage=template, method="Bramich"
                )[0]
            except ValueError:
                diff = ois.optimal_system(
                    image=hdu[name].data.byteswap().newbyteorder(),
                    refimage=template.byteswap().newbyteorder(),
                    method="Bramich",
                )[0]
            hdu.insert(
                1, CompImageHDU(data=diff, header=hduls[i]["ALGN"].header, name="SUB")
            )
            outputs.append(hdu)
...
@click.option(
    ...
    "--method",
    default="ois",
    help="The subtraction method to use; ois or numpy (straight subtraction).",
)
@cli.operator
