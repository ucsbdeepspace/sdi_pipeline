...
def cluster_ratio(hduls, name="CAT", tablename="OBJ"):
    ...
    print(
        "{} out of {} points clustered".format(clustered_points, points),
...
def collate(
    ...
    if algorithm == "DBSCAN":
        print(
            "\n Collating using {} with {} coordinates, minpts={} and eps={}".format(
                algorithm, coords, minpts, eps
            )
        )
        cores, labels = dbscan(np.vstack(normirdf).T, eps=eps, min_samples=minpts)
    elif algorithm == "OPTICS":
        print(
            "\n Collating using {} with {} coordinates, minpts={}, xi={}, and maxeps={}".format(
                algorithm, coords, minpts, xi, maxeps
            )
        )
        labels = (
            OPTICS(min_samples=minpts, max_eps=maxeps, xi=xi)
            .fit(np.vstack(normirdf).T)
            .labels_
        )
    else:
    ...
    if obj_count == -1:
        print(
            "No clusters could be formed. Please try using more images, increasing eps, or changing algorithms."
        )
        return (hdul for hdul in hduls)
        ...
        for j in range(source_count):
            if labels[j] != -1:
                if cols[labels[j]] == -1:
                    ...
                    if show_collisions:
                        print(
                            "collision at hdul {}, object '{}' at sources {} {}".format(
                                i, labels[j], j, cols[labels[j]]
                            )
                        )
        if len(labels) != source_count:
            labels = labels[source_count:]
        table = [
            fits.Column(name="objects", format="I", array=np.array(cols), ascii=True)
        ]
...
@click.option(
    ...
    "--name",
    default="CAT",
    help="name of HDU for each HDUL containing catalog of sources",
)
@click.option(
    "-t", "--tablename", default="OBJ", help="name of TableHDU to store cluster data"
)
...
@click.option(
    ...
    default=["x", "y", "flux"],
    multiple=True,
    help="list of dimensions to cluster sources over, multiple custom coordinates may be specified using: -c dim1 -c dim2 etc.",
)
@click.option(
    "-a",
    "--algorithm",
    default="DBSCAN",
    type=click.Choice(["DBSCAN", "OPTICS"], case_sensitive=False),
    help="choice of 'DBSCAN' or 'OPTICS' clustering algorithm",
)
...
@click.option(
    ...
    "--eps",
    default=0.001,
    help="point neighborhood radius parameter, used only in DBSCAN",
)
@click.option(
    "-m",
    "--maxeps",
    default=np.inf,
    help="maximum neighborhood radius parameter, used only in OPTICS",
)
@click.option(
    "-x",
    "--xi",
    default=0.85,
    help="minimum steepness of cluster boundary, used only in OPTICS",
)
@click.option(
    "-f",
    "--collision_fix",
    default=False,
    help="allows for optional resolving of collisions which may result in increased runtime",
)
@click.option(
    "-s",
    "--show_collisions",
    default=False,
    help="Display colliding sources and their corresponding images and clusters",
)
@cli.operator
