...
def cluster_ratio(hduls, name="CAT", tablename="OBJ"):
    """
    Prints number of clustered sources out of total sources and the percent of clustered sources.
    Arguments:
    ...
    print("{} out of {} points clustered".format(clustered_points, points))
    print(clustered_points/points)


def collate(hduls, name="CAT", tablename="OBJ", coords=None, algorithm="DBSCAN", minpts=4, eps=0.001, maxeps=np.inf, x_i=0.85, collision_fix=False, show_collisions=False):
    ...
    Arguments:
        ...
        x_i -- minimum steepness of cluster boundary, used only in OPTICS
    ...
    if algorithm == "DBSCAN":
        ...
        labels = dbscan(np.vstack(normirdf).T, eps=eps, min_samples=minpts)[1]
    elif algorithm == "OPTICS":
        print('\n Collating using {} with {} coordinates, minpts={}, x_i={}, and maxeps={}'.format(algorithm, coords, minpts, x_i, maxeps))
        labels = OPTICS(min_samples=minpts, max_eps=maxeps,x_i=x_i).fit(np.vstack(normirdf).T).labels_
    else:
    ...
    if collision_fix is True:
        if not collisions:
            ...
            for collision in collisions:
                ...
                s_1 = [sources[collision[2]][coord] for coord in coords]
                s_2 = [sources[collision[3]][coord] for coord in coords]
                if dist_func(s_1, mean) < dist_func(s_2, mean):
...
@click.option("-x", "--x_i", default=0.85, help="minimum steepness of cluster boundary, used only in OPTICS")
...
def collate_cmd(hduls, name="CAT", tablename="OBJ", coords=["x", "y", "flux"], algorithm="DBSCAN", minpts=4, eps=0.001, maxeps=np.inf, x_i=0.85, collision_fix=False, show_collisions=False): # collate function wrapper
    ...
    return collate(hduls, name, tablename, coords, algorithm, minpts, eps, maxeps, x_i, collision_fix, show_collisions)
