...
def display(hduls, cats=None, color="green", size=40):
    ...
    for hdul, cat in zip(hduls, cats):
        ...
        for source in cat:
            ds9.set(
                "regions command {{circle {} {} {} #color={}}}".format(
                    source["x"], source["y"], size, color
                )
            )
...
@click.option(
    ...
    "--color",
    default="green",
    help="A string color to draw" "circles in, e.g. 'green'",
)
@click.option(
    "-s", "--size", default=40, help="The radius of the circles" "drawn", type=int
)
@cli.operator
