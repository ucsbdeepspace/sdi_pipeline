...
RABOUND = [12.44,11.97,11.48,11,10.52,10.04,9.56]   #Bounds used to define sections 
_RASCALE = [-1,0,9,18,27,36,45]                     #Values to sum to find section ID according to a grid
...
def secid(hduls, read_ext='SCI', write_ext=-1):
    ...
    for hdul in hduls:
        ...
        for i,j in enumerate(RABOUND):
            if SCRA.value > RABOUND[i]: #Checks the values in a loop until it fails to be larger than one.
                RAid = _RASCALE[i]
                break
        ...
        if DECid != -1 and RAid != -1: #Creates the SECid if both the RA and DEC were found within bounds.
            SECid = DECid + RAid
        else:
            SECid = -1
        
        hdul[write_ext].header.set('SECID', SECid, comment='Setion ID in M31 as according to Lubin mosaic') #Writes the secid into the hdul
