...
class TestAlign(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.paths = glob.glob(
            os.path.join(
                ...
                "fixtures/science/tfn0m414-kb99-20180831-029[0-1]-e91.fits.fz",
            )
        )
        ...
        cls.path_len = len(
            os.path.join(
                os.path.dirname(__file__), "fixtures/comparativeData/alignData"
            )
        )
        cls.paths_true = glob.glob(
            "{}/*.fits*".format(
                os.path.join(
                    os.path.dirname(__file__), "fixtures/comparativeData/alignData"
                )
        ...
        cls.paths_true = sorted(
            cls.paths_true, key=lambda item: int(item[cls.path_len + 1 : len(item) - 5])
        )
    ...
    def test_length(self):
        self.assertEqual(
            len(TestAlign.output), 2, "Did not align the correct number of fits"
        )
