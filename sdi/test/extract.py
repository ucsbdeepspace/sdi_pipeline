...
class TestExtract(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.path_len = len(
            os.path.join(
                os.path.dirname(__file__), "fixtures/comparativeData/extractData"
            )
        )
        cls.paths = glob.glob(
            "{}/*.fits*".format(
    ...
    def test_length(self):
        self.assertEqual(
            len(TestExtract.output),
            10,
            "Did not extract the correct number of HDULs from fixtures/residuals",
        )
