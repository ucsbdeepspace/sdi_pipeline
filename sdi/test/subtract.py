...
class TestSubtract(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.path_len = len(
            os.path.join(
                os.path.dirname(__file__), "fixtures/comparativeData/subtractData"
            )
        )
        cls.paths = glob.glob(
            "{}/*.fits*".format(
        ...
        cls.paths = sorted(
            cls.paths, key=lambda item: int(item[cls.path_len + 1 : len(item) - 5])
        )
    ...
    def test_length(self):
        self.assertEqual(
            len(TestSubtract.output), 2, "Did not output the correct number of HDULs"
        )
