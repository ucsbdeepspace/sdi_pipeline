...
def normalize(flux,ref_flux,ref_mag):
    ...
    #If cat flux is longer
        ...
        return ref_mag-2.5*np.log10(flux[0:len(ref_flux)]/ref_flux)
...
def norm(hduls):
    ...
    try:
        ...
        c_best = np.linalg.lstsq(A, cat_flux[0:len(gaia_mag_transformed)], rcond=None)[0]
    #If cat_flux is shorter
    except ValueError:
        A = np.ones((len(cat_flux),1))
        A[:, 0] = 10**(-2.5*gaia_mag_transformed[0:len(cat_flux)])
    ...
    #If ref flux is longer
    try:
        mag_err = np.sqrt((gaia_mag_err)**2+((-2.5*cat_fluxerr)/(np.log(10)*cat_flux))**2+((2.5*gaia_flux_err)/(np.log(10)*gaia_flux_transformed[0:len(cat_flux)]))**2)
    #If cat flux is longer
    except ValueError:
        mag_err = np.sqrt((gaia_mag_err)**2+((-2.5*cat_fluxerr)/(np.log(10)*cat_flux[0:len(ref_mag)]))**2+((2.5*gaia_flux_err)/(np.log(10)*gaia_flux_transformed))**2)
